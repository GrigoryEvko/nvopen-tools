// Function: sub_2D2FBC0
// Address: 0x2d2fbc0
//
__int64 __fastcall sub_2D2FBC0(__int64 a1, unsigned int a2)
{
  __int64 v2; // r12
  unsigned __int64 *v4; // rax
  int v5; // r13d
  unsigned __int64 v6; // rdx
  unsigned __int64 v7; // rdi
  __int64 v8; // rax
  __int64 v9; // r13
  unsigned __int64 v10; // rdi
  int v11; // eax
  unsigned __int64 v13; // rsi

  v2 = a2;
  v4 = *(unsigned __int64 **)(a1 + 200);
  v5 = *(_DWORD *)(a1 + 196);
  v6 = *v4;
  if ( *v4 )
  {
    *v4 = *(_QWORD *)v6;
LABEL_3:
    memset((void *)v6, 0, 0xC0u);
    v7 = v6 & 0xFFFFFFFFFFFFFFC0LL;
    goto LABEL_4;
  }
  v13 = v4[1];
  v4[11] += 192LL;
  v7 = (v13 + 63) & 0xFFFFFFFFFFFFFFC0LL;
  if ( v4[2] < v7 + 192 || !v13 )
  {
    v6 = sub_9D1E70((__int64)(v4 + 1), 192, 192, 6);
    goto LABEL_3;
  }
  v4[1] = v7 + 192;
  if ( v7 )
  {
    v6 = (v13 + 63) & 0xFFFFFFFFFFFFFFC0LL;
    goto LABEL_3;
  }
LABEL_4:
  if ( v5 )
  {
    v8 = 1;
    v9 = (unsigned int)(v5 - 1);
    do
    {
      *(_QWORD *)(v6 + 8 * v8 - 8) = *(_QWORD *)(a1 + 8 * v8);
      *(_DWORD *)(v6 + 4 * v8 + 124) = *(_DWORD *)(a1 + 4 * v8 + 124);
      ++v8;
    }
    while ( v8 != v9 + 2 );
  }
  else
  {
    LODWORD(v9) = -1;
  }
  v10 = (unsigned int)v9 | v7;
  v11 = *(_DWORD *)((v10 & 0xFFFFFFFFFFFFFFC0LL) + 4LL * (unsigned int)v9 + 0x80);
  ++*(_DWORD *)(a1 + 192);
  *(_QWORD *)(a1 + 8) = v10;
  *(_DWORD *)(a1 + 128) = v11;
  *(_DWORD *)(a1 + 196) = 1;
  return v2 << 32;
}
