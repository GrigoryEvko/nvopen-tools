// Function: sub_1615E90
// Address: 0x1615e90
//
void __fastcall sub_1615E90(__int64 a1, __int64 *a2)
{
  __int64 v2; // rdx
  unsigned __int64 v3[2]; // [rsp-E8h] [rbp-E8h] BYREF
  _BYTE v4[64]; // [rsp-D8h] [rbp-D8h] BYREF
  _BYTE *v5; // [rsp-98h] [rbp-98h]
  __int64 v6; // [rsp-90h] [rbp-90h]
  _BYTE v7[16]; // [rsp-88h] [rbp-88h] BYREF
  unsigned __int64 v8[2]; // [rsp-78h] [rbp-78h] BYREF
  _BYTE v9[16]; // [rsp-68h] [rbp-68h] BYREF
  _BYTE *v10; // [rsp-58h] [rbp-58h]
  __int64 v11; // [rsp-50h] [rbp-50h]
  _BYTE v12[72]; // [rsp-48h] [rbp-48h] BYREF

  if ( dword_4F9EB40 > 3 )
  {
    v2 = *a2;
    v3[1] = 0x800000000LL;
    v6 = 0x200000000LL;
    v8[1] = 0x200000000LL;
    v10 = v12;
    v3[0] = (unsigned __int64)v4;
    v5 = v7;
    v8[0] = (unsigned __int64)v9;
    v11 = 0;
    v12[0] = 0;
    (*(void (__fastcall **)(__int64 *, unsigned __int64 *))(v2 + 88))(a2, v3);
    sub_1615A50(a1, "Preserved", 9u, (__int64)a2, (__int64)v8);
    if ( v10 != v12 )
      _libc_free((unsigned __int64)v10);
    if ( (_BYTE *)v8[0] != v9 )
      _libc_free(v8[0]);
    if ( v5 != v7 )
      _libc_free((unsigned __int64)v5);
    if ( (_BYTE *)v3[0] != v4 )
      _libc_free(v3[0]);
  }
}
