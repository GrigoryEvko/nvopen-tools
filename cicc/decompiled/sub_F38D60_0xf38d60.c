// Function: sub_F38D60
// Address: 0xf38d60
//
__int64 __fastcall sub_F38D60(__int64 a1, __int64 a2, __int64 *a3)
{
  __int64 v4; // r14
  int v5; // r13d
  bool v6; // zf
  __int64 v7; // r15
  unsigned int v8; // eax
  unsigned int v9; // r8d
  int v11; // eax
  bool v12; // al
  __int64 v13; // [rsp+20h] [rbp-C0h]
  int v14; // [rsp+28h] [rbp-B8h]
  unsigned int v15; // [rsp+2Ch] [rbp-B4h]
  int v16; // [rsp+3Ch] [rbp-A4h] BYREF
  __int64 v17; // [rsp+40h] [rbp-A0h] BYREF
  __int64 v18; // [rsp+48h] [rbp-98h] BYREF
  _QWORD v19[3]; // [rsp+50h] [rbp-90h] BYREF
  char v20; // [rsp+68h] [rbp-78h]
  __int64 v21; // [rsp+70h] [rbp-70h]
  _QWORD v22[12]; // [rsp+80h] [rbp-60h] BYREF

  if ( (*(_BYTE *)(a1 + 8) & 1) != 0 )
  {
    v4 = a1 + 16;
    v5 = 3;
  }
  else
  {
    v11 = *(_DWORD *)(a1 + 24);
    v4 = *(_QWORD *)(a1 + 16);
    v5 = v11 - 1;
    if ( !v11 )
    {
      *a3 = 0;
      return 0;
    }
  }
  v6 = *(_BYTE *)(a2 + 24) == 0;
  v20 = 0;
  v19[0] = 0;
  v21 = 0;
  memset(v22, 0, 24);
  v22[3] = 1;
  v22[4] = 0;
  v16 = 0;
  if ( !v6 )
    v16 = *(unsigned __int16 *)(a2 + 16) | (*(_DWORD *)(a2 + 8) << 16);
  v18 = *(_QWORD *)(a2 + 32);
  v17 = *(_QWORD *)a2;
  v14 = 1;
  v13 = 0;
  v15 = v5 & sub_F11290(&v17, &v16, &v18);
  while ( 1 )
  {
    v7 = v4 + 40LL * v15;
    LOBYTE(v8) = sub_F34140(a2, v7);
    v9 = v8;
    if ( (_BYTE)v8 )
    {
      *a3 = v7;
      return v9;
    }
    if ( sub_F34140(v7, (__int64)v19) )
      break;
    v12 = sub_F34140(v7, (__int64)v22);
    if ( !v13 )
    {
      if ( !v12 )
        v7 = 0;
      v13 = v7;
    }
    v15 = v5 & (v14 + v15);
    ++v14;
  }
  v9 = 0;
  if ( v13 )
    v7 = v13;
  *a3 = v7;
  return v9;
}
