// Function: sub_15C8760
// Address: 0x15c8760
//
__int64 __fastcall sub_15C8760(__int64 *a1, __int64 a2)
{
  __int64 v2; // rax
  __int64 v3; // rax
  __int64 result; // rax
  __int64 v5; // rdx
  __int64 (__fastcall *v6)(__int64 *, _QWORD **); // rax
  __int64 v7; // rax
  __int64 v8; // [rsp+0h] [rbp-80h] BYREF
  _QWORD *v9; // [rsp+8h] [rbp-78h]
  _QWORD *v10; // [rsp+10h] [rbp-70h] BYREF
  __int64 v11; // [rsp+18h] [rbp-68h]
  _QWORD v12[2]; // [rsp+20h] [rbp-60h] BYREF
  _QWORD *v13; // [rsp+30h] [rbp-50h] BYREF
  __int64 v14; // [rsp+38h] [rbp-48h]
  __int64 v15; // [rsp+40h] [rbp-40h]
  __int64 v16; // [rsp+48h] [rbp-38h]
  int v17; // [rsp+50h] [rbp-30h]
  __int64 *v18; // [rsp+58h] [rbp-28h]

  if ( (*(unsigned __int8 (__fastcall **)(__int64 *))(*a1 + 16))(a1) )
  {
    LOBYTE(v12[0]) = 0;
    v10 = v12;
    v11 = 0;
    v17 = 1;
    v13 = &unk_49EFBE0;
    v16 = 0;
    v15 = 0;
    v14 = 0;
    v18 = (__int64 *)&v10;
    v7 = sub_16E4080(a1);
    sub_16E59D0(a2, v7, &v13);
    if ( v16 != v14 )
      sub_16E7BA0(&v13);
    v8 = *v18;
    v9 = (_QWORD *)v18[1];
    (*(void (__fastcall **)(__int64 *, __int64 *, _QWORD))(*a1 + 216))(a1, &v8, 0);
    result = sub_16E7BC0(&v13);
    if ( v10 != v12 )
      return j_j___libc_free_0(v10, v12[0] + 1LL);
  }
  else
  {
    v2 = *a1;
    v8 = 0;
    v9 = 0;
    (*(void (__fastcall **)(__int64 *, __int64 *, _QWORD))(v2 + 216))(a1, &v8, 0);
    v3 = sub_16E4080(a1);
    result = sub_16E59E0(v8, v9, v3, a2);
    v11 = v5;
    v10 = (_QWORD *)result;
    if ( v5 )
    {
      v6 = *(__int64 (__fastcall **)(__int64 *, _QWORD **))(*a1 + 232);
      LOWORD(v15) = 261;
      v13 = &v10;
      return v6(a1, &v13);
    }
  }
  return result;
}
