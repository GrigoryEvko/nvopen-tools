// Function: sub_2550C90
// Address: 0x2550c90
//
void __fastcall sub_2550C90(unsigned __int64 a1)
{
  __int64 v2; // r15
  __int64 v3; // rsi
  unsigned __int64 v4; // rdi
  __int64 **v5; // rbx
  __int64 *v6; // rax
  __int64 v7; // rcx
  __int64 v8; // rdx
  __int64 v9; // rsi
  int v10; // edi
  __int64 v11; // rdx
  __int64 v12; // rcx
  __int64 v13; // r8
  __int64 *v14; // rax
  __int64 v15; // [rsp+8h] [rbp-88h]
  __int64 v16; // [rsp+18h] [rbp-78h]
  __int64 v17; // [rsp+18h] [rbp-78h]
  _QWORD v18[4]; // [rsp+20h] [rbp-70h] BYREF
  _QWORD v19[10]; // [rsp+40h] [rbp-50h] BYREF

  *(_QWORD *)a1 = &unk_4A1D4C0;
  *(_QWORD *)(a1 + 88) = &unk_4A1D550;
  if ( !*(_DWORD *)(a1 + 192) )
  {
    v2 = *(_QWORD *)(a1 + 176);
    v3 = 0;
    goto LABEL_3;
  }
  if ( !byte_4FEF240[0] && (unsigned int)sub_2207590((__int64)byte_4FEF240) )
  {
    unk_4FEF260 = -4096;
    unk_4FEF268 = -4096;
    unk_4FEF270 = 0;
    unk_4FEF278 = 0;
    sub_2207640((__int64)byte_4FEF240);
  }
  if ( !byte_4FEF208[0] && (unsigned int)sub_2207590((__int64)byte_4FEF208) )
  {
    unk_4FEF220 = -8192;
    unk_4FEF228 = -8192;
    qword_4FEF230 = 0;
    unk_4FEF238 = 0;
    sub_2207640((__int64)byte_4FEF208);
  }
  v5 = *(__int64 ***)(a1 + 176);
  v3 = *(unsigned int *)(a1 + 192);
  v2 = (__int64)&v5[v3];
  if ( v5 != &v5[v3] )
  {
    while ( 1 )
    {
      v6 = *v5;
      v7 = **v5;
      v8 = (*v5)[1];
      if ( unk_4FEF268 != v8 || unk_4FEF260 != v7 )
        goto LABEL_11;
      v9 = v6[2];
      if ( v9 != unk_4FEF270 )
      {
        if ( v9 == -4096 || unk_4FEF270 == -8192 || unk_4FEF270 == -4096 || v9 == -8192 )
          goto LABEL_11;
        if ( v9 )
        {
          v10 = *(_DWORD *)(v9 + 20) - *(_DWORD *)(v9 + 24);
          if ( unk_4FEF270 )
          {
            v16 = unk_4FEF270;
            if ( v10 != *(_DWORD *)(unk_4FEF270 + 20LL) - *(_DWORD *)(unk_4FEF270 + 24LL) )
              goto LABEL_11;
            if ( v10 )
            {
              v15 = *(_QWORD *)(v9 + 8);
              v18[1] = sub_254BB00(v9);
              v18[0] = v15;
              sub_254BBF0((__int64)v18);
              v18[2] = v9;
              v18[3] = *(_QWORD *)v9;
              v19[0] = sub_254BB00(v9);
              v19[1] = v19[0];
              sub_254BBF0((__int64)v19);
              v13 = v16;
              v19[2] = v9;
              v19[3] = *(_QWORD *)v9;
              v14 = (__int64 *)v18[0];
              if ( v19[0] != v18[0] )
              {
                while ( 1 )
                {
                  v17 = v13;
                  if ( !(unsigned __int8)sub_B19060(v13, *v14, v11, v12) )
                    break;
                  v18[0] += 8LL;
                  sub_254BBF0((__int64)v18);
                  v14 = (__int64 *)v18[0];
                  v13 = v17;
                  if ( v18[0] == v19[0] )
                    goto LABEL_14;
                }
                v6 = *v5;
                v7 = **v5;
                v8 = (*v5)[1];
LABEL_11:
                if ( unk_4FEF228 == v8 && unk_4FEF220 == v7 )
                  sub_254C7C0((__int64 *)v6[2], qword_4FEF230);
              }
            }
          }
          else if ( v10 )
          {
            goto LABEL_11;
          }
        }
        else if ( unk_4FEF270 && *(_DWORD *)(unk_4FEF270 + 24LL) != *(_DWORD *)(unk_4FEF270 + 20LL) )
        {
          goto LABEL_11;
        }
      }
LABEL_14:
      if ( (__int64 **)v2 == ++v5 )
      {
        v2 = *(_QWORD *)(a1 + 176);
        v3 = *(unsigned int *)(a1 + 192);
        break;
      }
    }
  }
LABEL_3:
  sub_C7D6A0(v2, v3 * 8, 8);
  v4 = *(_QWORD *)(a1 + 104);
  if ( v4 != a1 + 120 )
    _libc_free(v4);
  *(_QWORD *)a1 = &unk_4A16C00;
  sub_254FD20(a1 + 8);
  j_j___libc_free_0(a1);
}
