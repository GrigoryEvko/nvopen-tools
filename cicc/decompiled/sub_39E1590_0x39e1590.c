// Function: sub_39E1590
// Address: 0x39e1590
//
_BYTE *__fastcall sub_39E1590(__int64 a1)
{
  __int64 v2; // rdi
  __m128i *v3; // rdx
  unsigned __int64 v4; // r13
  _BYTE *result; // rax
  __int64 v6; // rdi
  __int64 v7; // r14
  char *v8; // rsi
  size_t v9; // rdx
  void *v10; // rdi

  v2 = *(_QWORD *)(a1 + 272);
  v3 = *(__m128i **)(v2 + 24);
  if ( *(_QWORD *)(v2 + 16) - (_QWORD)v3 <= 0xFu )
  {
    sub_16E7EE0(v2, "\t.cv_stringtable", 0x10u);
  }
  else
  {
    *v3 = _mm_load_si128((const __m128i *)&xmmword_3F7F880);
    *(_QWORD *)(v2 + 24) += 16LL;
  }
  v4 = *(unsigned int *)(a1 + 312);
  if ( *(_DWORD *)(a1 + 312) )
  {
    v7 = *(_QWORD *)(a1 + 272);
    v8 = *(char **)(a1 + 304);
    v9 = *(unsigned int *)(a1 + 312);
    v10 = *(void **)(v7 + 24);
    if ( v4 > *(_QWORD *)(v7 + 16) - (_QWORD)v10 )
    {
      sub_16E7EE0(*(_QWORD *)(a1 + 272), v8, v9);
    }
    else
    {
      memcpy(v10, v8, v9);
      *(_QWORD *)(v7 + 24) += v4;
    }
  }
  *(_DWORD *)(a1 + 312) = 0;
  if ( (*(_BYTE *)(a1 + 680) & 1) != 0 )
    return sub_39E0440(a1);
  v6 = *(_QWORD *)(a1 + 272);
  result = *(_BYTE **)(v6 + 24);
  if ( (unsigned __int64)result >= *(_QWORD *)(v6 + 16) )
    return (_BYTE *)sub_16E7DE0(v6, 10);
  *(_QWORD *)(v6 + 24) = result + 1;
  *result = 10;
  return result;
}
