// Function: sub_4A2500
// Address: 0x4a2500
//
__int64 __fastcall sub_4A2500(__int64 a1, const char *a2, _BYTE *a3, int **a4, __int64 *a5)
{
  int v8; // edx
  __int64 v9; // rax
  __int64 v10; // rdx
  size_t v11; // rax
  __int64 v12; // rdx
  int v13; // eax
  __int64 v15; // [rsp+8h] [rbp-38h]

  *(_QWORD *)a1 = &unk_49DC150;
  v8 = _InterlockedExchangeAdd64((volatile signed __int64 *)sub_C523C0(), 1u);
  *(_DWORD *)(a1 + 12) &= 0x8000u;
  *(_WORD *)(a1 + 16) = 0;
  *(_QWORD *)(a1 + 80) = 0x100000000LL;
  *(_DWORD *)(a1 + 8) = v8;
  *(_QWORD *)(a1 + 72) = a1 + 88;
  *(_QWORD *)(a1 + 24) = 0;
  *(_QWORD *)(a1 + 32) = 0;
  *(_QWORD *)(a1 + 40) = 0;
  *(_QWORD *)(a1 + 48) = 0;
  *(_QWORD *)(a1 + 56) = 0;
  *(_QWORD *)(a1 + 64) = 0;
  *(_QWORD *)(a1 + 96) = 0;
  *(_QWORD *)(a1 + 104) = a1 + 128;
  *(_QWORD *)(a1 + 112) = 1;
  *(_DWORD *)(a1 + 120) = 0;
  *(_BYTE *)(a1 + 124) = 1;
  v9 = sub_C57470();
  v10 = *(unsigned int *)(a1 + 80);
  if ( v10 + 1 > (unsigned __int64)*(unsigned int *)(a1 + 84) )
  {
    v15 = v9;
    sub_C8D5F0(a1 + 72, a1 + 88, v10 + 1, 8);
    v10 = *(unsigned int *)(a1 + 80);
    v9 = v15;
  }
  *(_QWORD *)(*(_QWORD *)(a1 + 72) + 8 * v10) = v9;
  ++*(_DWORD *)(a1 + 80);
  *(_DWORD *)(a1 + 136) = 0;
  *(_QWORD *)(a1 + 144) = &unk_49D9728;
  *(_QWORD *)a1 = off_49E6430;
  *(_QWORD *)(a1 + 160) = off_49E6400;
  *(_QWORD *)(a1 + 192) = nullsub_407;
  *(_DWORD *)(a1 + 152) = 0;
  *(_BYTE *)(a1 + 156) = 0;
  *(_QWORD *)(a1 + 184) = sub_11D98E0;
  v11 = strlen(a2);
  sub_C53080(a1, a2, v11);
  v12 = *a5;
  *(_BYTE *)(a1 + 12) = (32 * (*a3 & 3)) | *(_BYTE *)(a1 + 12) & 0x9F;
  v13 = **a4;
  *(_QWORD *)(a1 + 40) = v12;
  *(_BYTE *)(a1 + 156) = 1;
  *(_DWORD *)(a1 + 136) = v13;
  *(_DWORD *)(a1 + 152) = v13;
  *(_QWORD *)(a1 + 48) = a5[1];
  return sub_C53130(a1);
}
