// Function: sub_2509FE0
// Address: 0x2509fe0
//
__m128i *__fastcall sub_2509FE0(__m128i *a1, __int64 a2)
{
  char v3; // al
  __int64 *(__fastcall *v4)(__int64 *); // rax
  __int64 v5; // rax
  _BYTE *v6; // rdx
  __m128i *v7; // rax
  __int64 v8; // rcx
  _QWORD *v9; // rdi
  size_t v11; // [rsp+8h] [rbp-68h] BYREF
  _BYTE *v12; // [rsp+10h] [rbp-60h] BYREF
  size_t v13; // [rsp+18h] [rbp-58h]
  _QWORD v14[2]; // [rsp+20h] [rbp-50h] BYREF
  __int64 v15[2]; // [rsp+30h] [rbp-40h] BYREF
  __int64 v16; // [rsp+40h] [rbp-30h] BYREF

  v3 = sub_2509800((_QWORD *)(*(_QWORD *)a2 + 72LL));
  sub_2509010(v15, v3);
  v4 = *(__int64 *(__fastcall **)(__int64 *))(**(_QWORD **)a2 + 72LL);
  if ( v4 == sub_2508850 )
  {
    v11 = 21;
    v12 = v14;
    v5 = sub_22409D0((__int64)&v12, &v11, 0);
    v12 = (_BYTE *)v5;
    v14[0] = v11;
    *(__m128i *)v5 = _mm_load_si128((const __m128i *)&xmmword_4389C00);
    v6 = v12;
    *(_DWORD *)(v5 + 16) = 1953066089;
    *(_BYTE *)(v5 + 20) = 121;
    v13 = v11;
    v6[v11] = 0;
  }
  else
  {
    v4((__int64 *)&v12);
  }
  v7 = (__m128i *)sub_2241130((unsigned __int64 *)v15, 0, 0, v12, v13);
  a1->m128i_i64[0] = (__int64)a1[1].m128i_i64;
  if ( (__m128i *)v7->m128i_i64[0] == &v7[1] )
  {
    a1[1] = _mm_loadu_si128(v7 + 1);
  }
  else
  {
    a1->m128i_i64[0] = v7->m128i_i64[0];
    a1[1].m128i_i64[0] = v7[1].m128i_i64[0];
  }
  v8 = v7->m128i_i64[1];
  v7->m128i_i64[0] = (__int64)v7[1].m128i_i64;
  v9 = v12;
  v7->m128i_i64[1] = 0;
  a1->m128i_i64[1] = v8;
  v7[1].m128i_i8[0] = 0;
  if ( v9 != v14 )
    j_j___libc_free_0((unsigned __int64)v9);
  if ( (__int64 *)v15[0] != &v16 )
    j_j___libc_free_0(v15[0]);
  return a1;
}
