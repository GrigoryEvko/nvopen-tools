// Function: sub_CD5FF0
// Address: 0xcd5ff0
//
void __fastcall sub_CD5FF0(__int64 a1, unsigned __int64 a2)
{
  _DWORD *v3; // rax
  const void *v4; // r15
  __int64 v5; // rbx
  unsigned __int64 v6; // r14
  unsigned __int64 v7; // rax
  bool v8; // cf
  unsigned __int64 v9; // rax
  signed __int64 v10; // r8
  __int64 v11; // r9
  char *v12; // rcx
  char *v13; // rax
  _DWORD *v14; // rdx
  char *v15; // rax
  __int64 v16; // rsi
  __int64 v17; // r9
  __int64 v18; // rax
  __int64 v19; // [rsp-48h] [rbp-48h]
  __int64 v20; // [rsp-40h] [rbp-40h]
  char *v21; // [rsp-40h] [rbp-40h]
  __int64 v22; // [rsp-40h] [rbp-40h]

  if ( a2 )
  {
    v3 = *(_DWORD **)(a1 + 8);
    v4 = *(const void **)a1;
    v5 = (__int64)v3 - *(_QWORD *)a1;
    v6 = 0xAAAAAAAAAAAAAAABLL * (v5 >> 2);
    if ( a2 > 0xAAAAAAAAAAAAAAABLL * ((__int64)(*(_QWORD *)(a1 + 16) - (_QWORD)v3) >> 2) )
    {
      if ( 0xAAAAAAAAAAAAAAALL - v6 < a2 )
        sub_4262D8((__int64)"vector::_M_default_append");
      v7 = 0xAAAAAAAAAAAAAAABLL * (((__int64)v3 - *(_QWORD *)a1) >> 2);
      if ( a2 >= v6 )
        v7 = a2;
      v8 = __CFADD__(v6, v7);
      v9 = v6 + v7;
      if ( v8 )
      {
        v17 = 0x7FFFFFFFFFFFFFF8LL;
      }
      else
      {
        if ( !v9 )
        {
          v10 = v5;
          v11 = 0;
          v12 = 0;
LABEL_9:
          v13 = &v12[v5];
          do
          {
            *(_DWORD *)v13 = 0;
            v13 += 12;
            *((_DWORD *)v13 - 2) = 0;
            *((_DWORD *)v13 - 1) = 0;
          }
          while ( &v12[12 * a2 + v5] != v13 );
          if ( v10 > 0 )
          {
            v20 = v11;
            v15 = (char *)memmove(v12, v4, v10);
            v11 = v20;
            v12 = v15;
            v16 = *(_QWORD *)(a1 + 16) - (_QWORD)v4;
          }
          else
          {
            if ( !v4 )
            {
LABEL_13:
              *(_QWORD *)a1 = v12;
              *(_QWORD *)(a1 + 16) = v11;
              *(_QWORD *)(a1 + 8) = &v12[12 * v6 + 12 * a2];
              return;
            }
            v16 = *(_QWORD *)(a1 + 16) - (_QWORD)v4;
          }
          v19 = v11;
          v21 = v12;
          j_j___libc_free_0(v4, v16);
          v11 = v19;
          v12 = v21;
          goto LABEL_13;
        }
        if ( v9 > 0xAAAAAAAAAAAAAAALL )
          v9 = 0xAAAAAAAAAAAAAAALL;
        v17 = 12 * v9;
      }
      v22 = v17;
      v18 = sub_22077B0(v17);
      v4 = *(const void **)a1;
      v12 = (char *)v18;
      v11 = v18 + v22;
      v10 = *(_QWORD *)(a1 + 8) - *(_QWORD *)a1;
      goto LABEL_9;
    }
    v14 = &v3[3 * a2];
    do
    {
      *v3 = 0;
      v3 += 3;
      *(v3 - 2) = 0;
      *(v3 - 1) = 0;
    }
    while ( v14 != v3 );
    *(_QWORD *)(a1 + 8) = v14;
  }
}
